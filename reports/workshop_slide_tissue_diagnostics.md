...
Generated from repository-local manifests and canonical inference artifacts on 2026-08-31. This is a read-only, descriptive scientific analysis. No run, checkpoint, configuration, adapter, canonical registry, or scheduler state was changed or queried.
...
The generator contains FullFT discovery specifications and feeds FullFT through the same aggregation, rank, per-class, confusion, and CSV schema. Re-run with `--include-fullft` after the matched campaign is ready; no analysis-code or report-schema change is required. Any currently available FullFT artifacts are not mixed into this PEFT-only Part B snapshot.
